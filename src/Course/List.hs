...
find ::
  ...
  -> List a
  -> Optional a
-- find p list = case filter p list of
--   Empty    -> Empty
--   (h :. _) -> Full h
find _ Nil      = Empty
...
-- reverse = foldRight (flip (++) . (:. Nil)) Nil
reverse = foldLeft (flip (:.)) Nil

-- Tail recursive transformation of reverse
-- create new sub function with return value added as new argument
-- reverse = reverse' Nil
--   where
--     reverse' acc Nil      = acc
