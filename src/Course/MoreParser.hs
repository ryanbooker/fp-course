...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok x y pa = between (charTok x) (charTok y) (pa <* spaces)
...
hex ::
  Parser Char
hex = (?? chr 0) . ((<$>) chr) . readHex <$> (replicateA 4 (satisfy isHexDigit))
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma l r a = betweenCharTok l r (sepby a (charTok ','))
